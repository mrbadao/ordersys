...
$('#fileupload').fileupload({
    ...
    add: function (e, data) {
        var rows = $();
        var uploadErrors = [];
        var acceptFileTypes = /^video\/(mp4|avi|quicktime|3gpp)$/i;
        for(var checkFiles=0;checkFiles<data.originalFiles.length;checkFiles++){
            if (!acceptFileTypes.test(data.originalFiles[checkFiles]['type'])) {
                uploadErrors.push('Not an accepted file type');
            }
            if (data.originalFiles[checkFiles]['size'] > 10 * 1024 * 1024) {
        ...
        $.each(data.files, function (index, file) {
            var row = $('<tr class="template-upload" id="' + key + '">' +
                '<td><span class="preview"><div style="position: relative;margin-top: 20px;">' +
                '<span style="position: absolute;" class="loading"></span> ' +
                '<video style="display: none;overflow: hidden;" class="video_preview" id="img_' + key + '" controls><video/></div></span></td>' +
                '<td><div class="row">' +
                ...
                '</div></div>' +
                '<div class="row">' +
                '<label class="col-lg-2">Description <span class="txtWarning">*</span></label>' +
                '<div class="col-lg-10"><textarea class="form-control" rows="2" id="des" name="des"></textarea></div>' +
                '</div>' +
                '<div class="row">' +
                '<label class="col-lg-2">Youtube URL</label>' +
                ...
                '<div class="col-lg-6"><input type="checkbox" name="is_video" value="1" id="is_video"/></div>' +
                '</div>' +
                '</td>' +
//                '<td><p class="size"></p>' +
            ...
            if (k < data_input.length) {
                ...
                if (n == id) {
                    ...
                    data.formData = a[id] = {
                        ...
                        description: row.find('textarea#des').val(),
                        video: row.find("input#is_video").is(':checked') ? 1 : 0,
                        key: id
                    };
                    k++
                }
...
function validateForm() {
    ...
    for (var i = 0; i < data_input.length; i++) {
        ...
        if (tag == null || tag == "") {
            row.find('div.tagsinput').addClass('validate');
            hasError = true;
